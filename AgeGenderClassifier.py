...
# ===== Data Loading =====
def load_data_custom_cslu(cls, path):
    tr = os.path.join(path, cls, 'train')
    dv = os.path.join(path, cls, 'development')
    return DatasetDict({'train': Dataset.load_from_disk(tr), 'development': Dataset.load_from_disk(dv)})
...
def train_age_gender_classifier(args):
    ...
    model_name = args.model_name or 'classifier'
    results = {'epochs': [], 'accuracy_age': [], 'accuracy_gender': [], 'accuracy_joint': []}
    json_path = os.path.join(args.output_dir, 'results', 'classifier')
    os.makedirs(json_path, exist_ok=True)
    json_file = os.path.join(json_path, f"{model_name}.json")
    ...
    for epoch in trange(args.num_train_epochs, desc='Epochs'):
        ...
        ag_l, ag_p, gn_l, gn_p, j_l, j_p = eval_loop(model, dev_loader, device)

        acc_age = accuracy_score(ag_l, ag_p) if args.task in ('age', 'both') else None
        acc_gen = accuracy_score(gn_l, gn_p) if args.task in ('gender', 'both') else None
        acc_joint = accuracy_score(j_l, j_p) if args.task == 'both' else None

        # record
        results['epochs'].append(epoch+1)
        results['accuracy_age'].append(acc_age)
        results['accuracy_gender'].append(acc_gen)
        results['accuracy_joint'].append(acc_joint)
        # save interim JSON
        with open(json_file, 'w') as jf:
            json.dump(results, jf, indent=2)

        # print
        msg = f"Epoch {epoch+1}/{args.num_train_epochs} | Loss: {train_loss:.4f}"
        if acc_age is not None: msg += f" | Age Acc: {acc_age:.3f}"
        if acc_gen is not None: msg += f" | Gender Acc: {acc_gen:.3f}"
        if acc_joint is not None: msg += f" | Joint Acc: {acc_joint:.3f}"
        print(msg)

        # scheduler and best model on joint or single
        metric = acc_joint if args.task=='both' else (acc_age if args.task=='age' else acc_gen)
        scheduler.step(metric)
        if metric and metric > best_metric:
            best_metric = metric
            patience_counter = 0
            torch.save(model.state_dict(), os.path.join(args.output_dir, f"{model_name}.pt"))
            print(f"Saved best model ({metric:.3f})")
        else:
            patience_counter += 1
    ...
    # final report
    if args.task == 'both':
        print(classification_report(j_l, j_p, target_names=true_class_names, digits=3))
...
def main():
    ...
    parser.add_argument('--model_name', type=str, default="defaultmodel")
